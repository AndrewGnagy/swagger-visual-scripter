...
<br>A javascript library to create flowcharts
...
Based off [Flowy](https://github.com/alyssaxuu/flowy) made by [Alyssa X](https://alyssax.com)
...
## Installation
Adding Flowy-v2 to your WebApp is incredibly simple:

1.  Move  `flowy-v2.min.js`  and  `flowy-v2.min.css`  to your project.: 
```html
<link rel="stylesheet" href="flowy-v2.min.css"> 
<script src="flowy-v2.min.js"></script>
```
...
### Delete block
To remove a single block use:
```javascript
flowy.deleteBlock(blockId)
```

### Delete branch
To remove a block and its sub-tree use:
```javascript
flowy.deleteBranch(blockId)
```

### Add block
To add a block use:
```javascript
flowy.add(blockEl, parentId)
```