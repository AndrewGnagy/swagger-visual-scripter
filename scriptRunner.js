...
function executeBlock(id, iterableItems) {
  ...
  try {
    ...
    if (getDataProperty(block['data'], 'method')) {
      ...
      switch (blockType) {
        case 'if':
          let ifResult = processExpression(
            chartProperties[id].properties[0].value
          );
          ...
          return;
        case 'for':
          let loopExpressionResult = processExpression(
            chartProperties[id].properties[0].value
          );
...
function executeApiBlock(block, iterableItems) {
  ...
  if (
    chartProperties[block.id] !== undefined &&
    chartProperties[block.id].properties !== undefined
  ) {
...
let swagLog = function (log) {
  ...
  document
    ...
    .insertAdjacentHTML('beforeend', logEntry);
};
