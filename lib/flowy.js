  ...
  flowy.load = function () {
    ...
    function moveChildrenToParentPos(id) {
      ...
      for (let i = 0; i < childern_block_idx.length; i++) {
        ...
        getBlock(child_block.id).style.top = getBlock(
          child_block.parent
        ).style.top;

        if (
          document.querySelector(".arrowid[value='" + child_block.id + "']")
        ) {
          getArrow(child_block.id).style.top = getArrow(
            child_block.parent
          ).style.top;
        }
      }
    ...
    flowy.deleteBlock = function (id) {
      if (id == 0) {
        ...
        let remove_block_elem = document.querySelector(
          ".blockid[value='" + id + "']"
        );
    ...
    function drawArrow(parent, x, y, id) {
      if (x < 0) {
        canvas_div.innerHTML +=
          ...
          (blocks.filter((a) => a.id == id)[0].x - parent.x + 5) +
          ' 0L' +
          (blocks.filter((a) => a.id == id)[0].x - parent.x + 5) +
          ' ' +
          paddingy / 2 +
    ...
    function updateArrow(parent, x, y, child) {
      let arrowEl = document.querySelector(
        '.arrowid[value="' + child.id + '"]'
      );
      if (x < 0) {
        arrowEl.parentNode.style.left =
          parent.x -
          5 -
          (absx + window.scrollX) +
          canvas_div.getBoundingClientRect().left +
          'px';
        arrowEl.parentNode.innerHTML =
          '<input type="hidden" class="arrowid" value="' +
          child.id +
          '"><svg preserveaspectratio="none" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M' +
          (blocks.filter((id) => id.id == child.parent)[0].x - parent.x + 5) +
          ' 0L' +
          (blocks.filter((id) => id.id == child.parent)[0].x - parent.x + 5) +
          ' ' +
          paddingy / 2 +
        ...
        arrowEl.parentNode.style.left =
          blocks.filter((id) => id.id == child.parent)[0].x -
          20 -
          (absx + window.scrollX) +
          canvas_div.getBoundingClientRect().left +
          'px';
    ...
    function snap(block, i, blocko) {
      ...
      for (
        ...
        var children = blocks.filter((id) => id.parent == blocko[i])[w];
        if (children.childwidth > children.width) {
          getBlock(children.id).style.left =
      ...
      if (rearrange) {
        blockstemp.filter(
          (a) => a.id == parseInt(block.querySelector('.blockid').value)
        )[0].x =
          block.getBoundingClientRect().left +
          window.scrollX +
          parseInt(window.getComputedStyle(block).width) / 2 +
          canvas_div.scrollLeft -
          canvas_div.getBoundingClientRect().left;
        blockstemp.filter(
          (a) => a.id == parseInt(block.querySelector('.blockid').value)
          ...
          window.scrollY +
          parseInt(window.getComputedStyle(block).height) / 2 +
          canvas_div.scrollTop -
          canvas_div.getBoundingClientRect().top;
        blockstemp.filter(
          (a) => a.id == block.querySelector('.blockid').value
        )[0].parent = blocko[i];
        for (var w = 0; w < blockstemp.length; w++) {
          if (
            blockstemp[w].id != parseInt(block.querySelector('.blockid').value)
          ) {
            const blockParent = getBlock(blockstemp[w].id);
            const arrowParent = getArrow(blockstemp[w].id);
            blockParent.style.left =
    ...
    function checkOffset() {
      ...
      if (
        ...
        for (var w = 0; w < blocks.length; w++) {
          getBlock(blocks.filter((a) => a.id == blocko[w])[0].id).style.left =
        ...
        for (var w = 0; w < blocks.length; w++) {
          blocks[w].x =
            getBlock(blocks[w].id).getBoundingClientRect().left +
            window.scrollX +
            canvas_div.scrollLeft +
            parseInt(window.getComputedStyle(getBlock(blocks[w].id)).width) /
              2 -
            20 -
            canvas_div.getBoundingClientRect().left;
        }
    ...
    function rearrangeMe() {
      ...
      for (var z = 0; z < blockParents.length; z++) {
        ...
        if (blockParents[z] != -1) {
          blocks.filter((a) => a.id == blockParents[z])[0].childwidth =
            totalwidth;
        }
        ...
        for (var w = 0; w < childBlocks.length; w++) {
          ...
          if (child.childwidth > child.width) {
            ...
            r_block.style.left =
              ...
              'px';
            child.x =
              r_array[0].x - totalwidth / 2 + totalremove + child.width / 2;
            totalremove += child.width + paddingx;
          }
          var arrowx =
            child.x - blocks.filter((a) => a.id == child.parent)[0].x + 20;
          var arrowy = paddingy;
